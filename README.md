...
| 認証 / 認可 | Spring Security 6.5.6（JWT） |
| O/Rマッパー | MyBatis 3.0.5 |
| マイグレーション | Flyway 11.7.2 |
| テスト | JUnit 5 |

#### 【選定理由】<br>
型安全性による保守性の高さと、実務での採用実績の多さから **Java / Spring Boot** を選定。<br>
また、エリア・カテゴリ・設備など複数条件を組み合わせる動的検索が必要なため、SQLを直接記述して柔軟に制御できる **MyBatis** を選定。
...
#### 【選定理由】<br>
コンポーネント単位でUIを管理しやすく、スムーズな検索体験（SPA）を提供できること、
また実務での採用実績が豊富であることから **React / TypeScript** を選定。<br>
スタイリングには、保守しやすく開発速度も高い **Tailwind CSS** を採用。<br>
...
| 分類 | 技術 |
|---|---|
| クラウド | AWS（Route53 / CloudFront / S3 / VPC / ALB / EC2 / RDS / ACM） |
| コンテナ | Docker（バックエンド） |
| データベース | MySQL |
| バージョン管理 | Git / GitHub |
...
| spot_facilities | お出かけスポットの設備情報を管理する（spots と 1:1 の関係） |
| reviews | ユーザーが投稿するレビュー情報を管理する（spots・users に紐づく） |
| favorites | ユーザーのお気に入りスポットを管理する（spots・users に紐づく） |
...
| 機能 | ログイン要否 |
|---|---|
| ユーザー登録（登録後、自動ログイン） | 不要 |
| ログイン | 不要 |
| ログアウト | 要 |
| ユーザー情報編集（表示名、メールアドレス、パスワードの変更） | 要 |
| ユーザー削除（退会） | 要 |
